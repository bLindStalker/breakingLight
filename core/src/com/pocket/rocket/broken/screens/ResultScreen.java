...
public class ResultScreen extends BaseScreen {
    ...
    public ResultScreen(final com.pocket.rocket.broken.Main main, int score, int starCollected, int time) {
        super(main);
        score -= time * 5;
    ...
    private Group buildResult(int score, int starCollected, int time) {
        ...
        resultGroup.addActor(new ImageActor((WIDTH - RESULT_BG_WIDTH) / 2, (HEIGHT - RESULT_BG_HEIGHT) / 2, RESULT_BG_WIDTH, RESULT_BG_HEIGHT, com.pocket.rocket.broken.AssetLoader.getResultBg()));
        ...
        resultGroup.addActor(new ImageActor(X_CENTER - 250, 540, 85, 85, com.pocket.rocket.broken.AssetLoader.getStar()));

        Label scoreLabel = new Label(": " + starCollected, com.pocket.rocket.broken.AssetLoader.getFont(com.pocket.rocket.broken.AssetLoader.BUTTON_TEXT_COLOR));
        scoreLabel.setAlignment(Align.left);
        scoreLabel.setBounds(X_CENTER - 150, 560, 100, 50);
        scoreLabel.setScale(0.6f, 0.6f);
        resultGroup.addActor(scoreLabel);

        TextButton menuButton = new TextButton("MENU", com.pocket.rocket.broken.AssetLoader.getButtonStyle());
        ...
        TextButton retryButton = new TextButton("RETRY", com.pocket.rocket.broken.AssetLoader.getButtonStyle());
    ...
    private Actor buildLabel(int time) {
        return time == 0
                ? new ImageActor(10, (HEIGHT) / 2 + 220, WIDTH - 20, 300, com.pocket.rocket.broken.AssetLoader.getCongratulationsLabel())
                : new ImageActor((WIDTH - GAME_OVER_WIDTH) / 2, (HEIGHT) / 2 + 140, GAME_OVER_WIDTH, GAME_OVER_HEIGHT, com.pocket.rocket.broken.AssetLoader.getGameOverLabel());
    }

    private void buildResultData(Group result, int xPosition, String header, String data) {
        Label scoreLabel = new Label(header, com.pocket.rocket.broken.AssetLoader.getFont(com.pocket.rocket.broken.AssetLoader.BUTTON_TEXT_COLOR));
        scoreLabel.setAlignment(Align.left);
        scoreLabel.setBounds(xPosition, 700, 100, 50);
        result.addActor(scoreLabel);

        Label scoreActor = new Label(data, com.pocket.rocket.broken.AssetLoader.getFont(FIREBRICK));
